...
public class GM : MonoBehaviour {
	...
	void Start () {
		...
		if (targetSlovingQuestionSwish)
		{
			...
			while (targetSlovingQuestionSwish)
			{
				...
				//變更起點節點圖示
				//int fi,fj;
				//findStartNodeType(QuestionNodeType,QuestionNodeSwish, out fi,out fj);
				//changTexture(fj,28,QuestionNodeObj_prefab[fi,fj]);
			}
		}
	...
	{	
		...
		if(Input.GetMouseButtonDown(0) && GuiNextQuession.enabled)
		{
			...
			{
/*!!*/			Application.LoadLevel("");  //<-------------------------------------------------------------------------設定到下一個題目場景
			}
		}
	...
	{	
		...
		changTexture(intoIDj,4,QuestionNodeObj_prefab[intoIDi,intoIDj]);
