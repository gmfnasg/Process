...
//隨機產生陣列並自動檢查是否可以達到終點
//目標-非單次階層左右位移解題模式
public class CreatArray11 : MonoBehaviour {
	...
	//將所有新陣列節點開關設為關閉(要關閉的陣列)
	void turnoffAllDisplacementNodeSwish(bool[,] swish) 
	{
	...
	void DisplacementAllLayerType()
	{
		...
		//產生新的節點陣列物件來顯示位移結果
		creatNewCubeAndSetup(0,0,6,DisplacementNodeType);
	}
	
	//位移指定J層整排(或列)節點屬性 (指定層,指定方向，0向右,1向左,2再往右,3再往左)
	void DisplacementOneLayerType(int layerID,int displacementDirection,int MovieX,int MovieY,int MovieZ,int[,] NewType, int[,] OldType)
	{
		//往右位移
		if(displacementDirection==0)
		{
			...
			creatNewCubeAndSetup(MovieX,MovieY,MovieZ,DisplacementNodeType);
		}
		...
		else if(displacementDirection==2)
		{
			...
			for(int i = 1;i<node+1;i++)
			{
				//print("開始往右位移存到新陣列");
				//將原始的屬性值記錄到新的陣列右邊那格
				NewType[i+1,layerID]=OldType[i,layerID];
				//狀況如下
				//012345 	<-tempType
				//  ↑↑↑↑
				// 012345 	<-DisplacementNodeType
			...
			//將紀新的陣列第一格紀錄為原始最右邊那格屬性
			NewType[1,layerID] = OldType[node,layerID];
			//狀況如下
			//012345 	<-DisplacementNodeType
			// ↑ 
			// 4 		<-NodeType
			...
			//產生新的節點陣列物件來顯示位移結果
			//print("開始利用新陣列屬性來產生新物件");
			creatNewCubeAndSetup(MovieX,MovieY,MovieZ,NewType);
		}
		
		//再再往右
		else if(displacementDirection==4)
		{
			//將資料記錄到新的陣列
			for(int i = 1;i<node+1;i++)
			{
			...
			//產生新的節點陣列物件來顯示位移結果
			//print("開始利用新陣列屬性來產生新物件");
			creatNewCubeAndSetup(MovieX,MovieY+layerID*layer,MovieZ,NewType);
			print("Y="+(MovieY+layerID*layer).ToString());
		}
	...
	void creatNewCubeAndSetup(int MoveX,int MoveY,int MoveZ,int[,] type) 
	{
		...
		for(int i = 0; i<node; i++)
		{
			for(int j = 0; j<layer; j++)
			{
				...
				//將節點依照屬性改變其貼圖圖示
				changTexture2(i,j,type[i+1,j],newCube_prefab);
	...
	//將新節點屬性設為原始題目節點屬性(0=第一次位移設定用 1=再位移設定用)
	void setDisplacementNodeType(int setType,int[,] newType,int[,] oldType)
	{
		if(setType == 0)
		{
			for(int j = 1;j<layer-1;j++)
	...
	void tryToDisplacementSlovingType(int SlovingType)
	{
		...
		//紀錄再次位移前的屬性
		int[,] tempType = new int[node+2,layer];
		bool[,] tempSwish = new bool[node+2,layer];
		...
		if(SlovingType==0)
		{
			...
			while (!slovingQuestionSwish)
			{
				...
				if(j<layer-1)
				{
					...
					//恢復起始題目類型屬性給新陣列
					setDisplacementNodeType(0,DisplacementNodeType,NodeType);
					...
					//位移節點來看嘗試解題可能性
					DisplacementOneLayerType(j,0,0,0,6+(j*6),DisplacementNodeType,NodeType);//X座標,Y座標,Z座標(+(往右第幾次位移*每個位移間距))
					...
					turnoffAllDisplacementNodeSwish(DisplacementNodeSwish);
					...
					//再次往右位移解題
					//print("!!- - - -　　　　開始再次往右位移解題");
					tryMoreDisplacementSlovingTypeA(0,j-1,DisplacementNodeType,tempType,tempSwish,0,0,0);
		...
		else if(SlovingType == 1)
		{
			...
			{	
				...
				if(j<layer-1)
				{
					...
					//位移節點來看嘗試解題可能性
					DisplacementOneLayerType(j,1,0,0,6+(j*6)+node*6+5,DisplacementNodeType,NodeType);//X座標,Y座標,Z座標(與起始座標間距+(往右第幾次位移*每個位移間距)+與所有往右位移間物件的距+擴大間距以利區分)
	...
	//再次位移解題(0=往右位移， 1=往左位移, 目前是第幾次位移, 舊屬性, 新屬性, 開關屬性, 整體位移X, 整體位移Y, 整體位移Z)
	void tryMoreDisplacementSlovingTypeA(int SlovingType,int DisplacementCount, int[,] oldType, int[,] NEWtempType, bool[,] tempSwish,int MovieX,int MovieY,int MovieZ)
	{
		//紀錄自動解題次數
		int countForCheckQuestion=1;
		//指定位移層ID用
		...
		{
			//開始再次往右位移自動解題
			//print("!!A    再次開始往右位移自動解題,j=" + j.ToString());
			countForCheckQuestion=0;//還原位移次數
			
	
			//如果指定位移層沒有超過最上層5
			for (int j2 = 1;j2<2;j2++)
			//for (int j2 = 1;j2<layer-1;j2++)
			{
				//print("!!A    再次開始自動往右位移,j=" + j.ToString());
				//將第一次類型屬性給新陣列
				setDisplacementNodeType(1,NEWtempType,oldType);
				
				//位移節點來看嘗試解題可能性
				DisplacementOneLayerType(j2,2,(node+2+(j2*(node+2)))*-1+MovieX,0+MovieY,layer+(DisplacementCount*layer)+MovieZ,NEWtempType,oldType);//X座標(往右第幾次位移*每個位移間距),Y座標,Z座標
				//DisplacementOneLayerType(j2,2,(6+(j2*6))*-1+MovieX,0+MovieY,6+(DisplacementCount*6)+MovieZ,NEWtempType,oldType);//X座標(往右第幾次位移*每個位移間距),Y座標,Z座標
				...
				//重置節點開關屬性
				turnoffAllDisplacementNodeSwish(tempSwish);
					
				//重新判定路線是否跑完
				checkRouteStatus = false;
				//檢查題目起始點最終會停到哪
				//print("!!A    檢查slovingQuestionSwish屬性為"+slovingQuestionSwish.ToString());
				//print("!!A    開始檢查第"+countForCheckQuestion.ToString()+"次再次往右移動完後是否能完成解題");
				checkQuestion(1);
				//print("!!A    檢查完後slovingQuestionSwish屬性為"+slovingQuestionSwish.ToString());
				countForCheckQuestion++;
				
				//再再次往右位移解題
				int[,] tempType2 = new int[node+2,layer];
				tryMoreDisplacementSlovingTypeB(0,DisplacementCount,NEWtempType,tempType2,tempSwish,0,layer+1,0);
			}
		}
	...
	//再次位移解題(0=往右位移， 1=往左位移, 目前是第幾次位移, 舊屬性, 新屬性, 開關屬性, 整體位移X, 整體位移Y, 整體位移Z)
	void tryMoreDisplacementSlovingTypeB(int SlovingType,int DisplacementCount, int[,] oldType, int[,] NEWtempType, bool[,] tempSwish,int MovieX,int MovieY,int MovieZ)
	{
		...
		if(SlovingType == 0)
		{
			...
			//紀錄自動解題次數
			int countForCheckQuestion=0;//還原位移次數
			//print("!!B    再次開始往右位移自動解題,countForCheckQuestion=" + countForCheckQuestion.ToString());
			...
			for (int j2 = 1;j2<layer-1;j2++)
			{
				//print("!!B    再次開始自動往右位移,countForCheckQuestion=" + countForCheckQuestion.ToString());
				//將第一次類型屬性給新陣列
				setDisplacementNodeType(1,NEWtempType,oldType);
				
				//位移節點來看嘗試解題可能性
				DisplacementOneLayerType(j2,4,(node+2+(j2*(node+2)))*-1+MovieX,MovieY,layer+(DisplacementCount*layer)+MovieZ,NEWtempType,oldType);//X座標(往右第幾次位移*每個位移間距),Y座標,Z座標
				//DisplacementOneLayerType(j2,2,(6+(countForCheckQuestion2*6))*-1+MovieX,0+MovieY,6+(DisplacementCount*6)+MovieZ,NEWtempType,oldType);//X座標(往右第幾次位移*每個位移間距),Y座標,Z座標
				
				//尋找檢查的起始節點IDij
				//findStartNodeType();
				//print ("!!B    找完起始節點後ID ij="+checkDisplacementNodeI.ToString()+checkDisplacementNodeJ.ToString());
				...
				//checkRouteStatus = false;
				//檢查題目起始點最終會停到哪
				//print("!!B    檢查slovingQuestionSwish屬性為"+slovingQuestionSwish.ToString());
				//print("!!B    開始檢查第"+countForCheckQuestion.ToString()+"次再次往右移動完後是否能完成解題");
				//checkQuestion(1);
				//print("!!B    檢查完後slovingQuestionSwish屬性為"+slovingQuestionSwish.ToString());
				print(countForCheckQuestion.ToString());
				countForCheckQuestion++;
