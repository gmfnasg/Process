...
//隨機產生陣列並自動檢查是否可以達到終點
//目標-非單次階層左右位移解題模式
public class CreatArray10 : MonoBehaviour {
	...
	void tryMoreDisplacementSlovingTypeB(int SlovingType,int DisplacementCount, int[,] oldType, int[,] NEWtempType, bool[,] tempSwish,int MovieX,int MovieY,int MovieZ)
	{
		...
		if(SlovingType == 0)
		{
			...
			for (int j2 = 1;j2<layer-1;j2++)
			{
				...
				//位移節點來看嘗試解題可能性
				DisplacementOneLayerType(j2,2,(node+2+(j2*(node+2)))*-1+MovieX,MovieY,layer+(DisplacementCount*layer)+MovieZ,NEWtempType,oldType);//X座標(往右第幾次位移*每個位移間距),Y座標,Z座標
