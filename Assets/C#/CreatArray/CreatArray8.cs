...
//隨機產生陣列並自動檢查是否可以達到終點
//目標-位移整排(或列)節點屬性
public class CreatArray8 : MonoBehaviour {
		...
		else if(DisplacementNodeType[checkDisplacementNodeI,checkDisplacementNodeJ]==1)
		{
			...
			if(checkDisplacementNodeI>1)
			{
				...
				else
				{
					...
					print("目前在i=" +checkDisplacementNodeI.ToString()+", j="+checkDisplacementNodeJ.ToString() + ", 往左移動節點到i="+checkDisplacementNodeI.ToString()+", j="+checkDisplacementNodeJ.ToString()+", Type="+DisplacementNodeType[checkDisplacementNodeI,checkDisplacementNodeJ].ToString());
					}
					else{
						checkRouteStatus = true;
						print ("重複迴圈");
			...
			else if(checkDisplacementNodeI == 1)
			{
				//檢查左邊的節點
				//無法前進時
				if(DisplacementNodeType[node-2,checkDisplacementNodeJ] == 3)
				{
					print("改之前checkRouteStatus="+checkRouteStatus.ToString());
					checkRouteStatus = true;
					print("改之後checkRouteStatus="+checkRouteStatus.ToString());
					print("目前在i=" +checkDisplacementNodeI.ToString()+", j="+checkDisplacementNodeJ.ToString() + ", 無法往左前進到i="+(node-1).ToString()+", j="+checkDisplacementNodeJ.ToString()+", Type="+DisplacementNodeType[node-1,checkDisplacementNodeJ].ToString());
				}
				else if(DisplacementNodeType[node-2,checkDisplacementNodeJ] == 0)
				{
					print("改之前checkRouteStatus="+checkRouteStatus.ToString());
					checkRouteStatus = true;
					print("改之後checkRouteStatus="+checkRouteStatus.ToString());
					print("目前在i=" +checkDisplacementNodeI.ToString()+", j="+checkDisplacementNodeJ.ToString() + ", 會跑進黑洞i="+(node-1).ToString()+", j="+checkDisplacementNodeJ.ToString());
				}
				...
				{
					//如果下個此節點沒有重複進入過
					if(!DisplacementNodeSwish[node-2,checkDisplacementNodeJ])
					{
						//將停留節點往左移動
						checkDisplacementNodeI=node-2;
						DisplacementNodeSwish[checkDisplacementNodeI,checkDisplacementNodeJ] = true;
						print("目前在i=0, j="+checkDisplacementNodeJ.ToString() + ", 往左移動節點到i="+checkDisplacementNodeI.ToString()+", j="+checkDisplacementNodeJ.ToString()+", Type="+DisplacementNodeType[checkDisplacementNodeI,checkDisplacementNodeJ].ToString());
					}
					else{
						checkRouteStatus = true;
						print ("重複迴圈");
	...
	void DisplacementOneLayerType(int layerID,int displacementDirection)
	{
		//往右位移
		if(displacementDirection==0)
		{
			...
			creatNewCubeAndSetup(0,0,6+(layerID*6));
		}
		//往左位移
		else if(displacementDirection==1)
		{
			...
			print("開始利用新陣列屬性來產生新物件");
			creatNewCubeAndSetup(0,0,6+(layerID*6)+node*6+5);//X座標,Y座標,Z座標(與起始座標間距+(往右第幾次位移*每個位移間距)+與所有往右位移間物件的距+擴大間距以利區分)
		}
